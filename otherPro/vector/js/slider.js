...
Util.Drag.prototype = {
	...
	init : function(container,drag,options){
		...
		this.Move = xhsilde.bindAsEventListener(this,this.move);
		this.Stop = xhsilde.bindAsEventListener(this,this.stop);
		this.drag.style.cursor = "move";	
		xhsilde.addEvent(this.drag,xhsilde.bindAsEventListener(this,this.start),"mousedown");
	},
	start : function(E){	
		this.x = E.clientX - this.drag.offsetLeft;
		this.y = E.clientY - this.drag.offsetTop;		
		xhsilde.addEvent(document,this.Move,"mousemove");
		xhsilde.addEvent(document,this.Stop,"mouseup");
		this.startFunc.call(this,E, this.container);
		if(document.all){
			xhsilde.addEvent(this.drag, this.Stop, "losecapture");
			this.drag.setCapture();
			E.cancelBubble = true;
		}else{
			xhsilde.addEvent(window, this.Stop, "blur");
			E.preventDefault();
		}
	},
	move : function(E){
	...
	stop : function(E){
		xhsilde.removeEvent(document,this.Move,"mousemove");
		xhsilde.removeEvent(document,this.Stop,"mouseup");
		this.endFunc.call(this,E, this.container);
		if(document.all){
			xhsilde.removeEvent(this.drag, this.Stop, "losecapture");
			this.drag.releaseCapture();
			E.cancelBubble = true;
		}else{
			xhsilde.removeEvent(window, this.Stop, "blur");
		}
	}
};
...
Slider.prototype = {
	...
	init : function(options){
		...
		this.icontainer = xhsilde.get(options.icontainer);
		this.idrag = xhsilde.get(options.idrag);
		if(options.plusBtn){
			this.plusBtn = xhsilde.get(options.plusBtn);
			this.reduceBtn = xhsilde.get(options.reduceBtn);			
		}	
		this.direction = options.direction;
		if(options.content){
			this.content = xhsilde.get(options.content);
			this.panel = xhsilde.get(options.panel);
	...
	addEvent : function(){
		...
		self.fixedBug();
		if(self.plusBtn){
			xhsilde.addEvent(self.plusBtn, function(){
				self.goToward("up", self);
			}, "mousedown");
			xhsilde.addEvent(self.plusBtn, function(){
				clearTimeout(self.timer);
			}, "mouseup");	
			xhsilde.addEvent(self.reduceBtn, function(){
		...
		xhsilde.addEvent(self.icontainer, function(event){
		...
		new Util.Drag(self.icontainer, self.idrag, {
			...
			moveFunc : function(E, idrag, icontainer){
				if(self.content){
					self.callback(parseInt(xhsilde.getStyle(idrag, self.direction)), self.Max[self.direction]);
				}
			},
			endFunc : function(E){		
				setTimeout(function(){
	...
	run : function (self){
		...
		var posUp = Math.min(Math.max(parseInt(xhsilde.getStyle(self.idrag, self.direction)) + acc,self.Min[self.direction]),self.Max[self.direction]);
		if(self.content){
	...
	mouseClick : function(event, self){
		var pos = xhsilde.getPos(event),
			offset = xhsilde.getS(self.idrag),
			dis = pos[self.direction == "top" ? "pageY" : "pageX"] - offset[self.direction],
			top = parseInt(xhsilde.getStyle(self.idrag, self.direction)),
		...
		if(self.content){
			self.callback(parseInt(xhsilde.getStyle(self.idrag, self.direction)), self.Max[self.direction]);	
		}
		xhsilde.preventDefault(event);	
	},	
	scrollFunc : function scrollFunc(event, self){
		self.dec = xhsilde.getDetail(event);
		self.isScroll = true;
		self.run();
		xhsilde.preventDefault(event);
	},	
	keydown : function scrollFunc1(event, self){
		var which = xhsilde.getWhich(event);
		if(which === 38){
			self.dec = - 10;
		}
	...
	fixedBug : function(){
		...
		if(!document.all){
			xhsilde.addEvent(this.icontainer, function(){
